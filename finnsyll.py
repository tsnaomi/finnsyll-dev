...
class Token(db.Model):
    ...
    # the word's orthography in lowercase, with umlauts replaced
...
# -------------------------------------------------------------------------
    ...
    # a boolean indicating if the word is likely a loanword
    ...
    def syllabify(self, gold_base=False, test_base=False):
        '''Programmatically syllabify the Token based on its base form.'''
        from syllabifier.v8 import syllabify

        if test_base:
            syllabifications = list(syllabify(self.test_base))
...
def get_unseen_lemmas():
    ...
    tokens = Token.query.filter_by(is_aamulehti=True, freq=0)
...
def get_false_negative_compounds():
    ...
    return get_gold_tokens().filter_by(is_complex=True, is_test_compound=False)
...
def main_view():
    ...
    # cacculate gold_accuracy
    complex_gold = GOLD.filter_by(is_complex=True).count()
    complex_verified = VERIFIED.filter_by(is_complex=True).count()
    complex_accuracy = (float(complex_gold) / complex_verified) * 100
    ...
    stats = {
        ...
        'complex_accuracy': round(complex_accuracy, 2),
