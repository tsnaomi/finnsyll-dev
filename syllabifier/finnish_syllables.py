...
# note: initial consonant cluster not listed in inseparable clusters will be
# split up (e.g., traffic -> .t.raf.fic.)

# for a word w, syllable boundaries are represented by a list l of length
# len(w)+1 l[i] = 1 iff w[i] should be preceded by a syllable boundary, else
# l[i] = 0 thus, the first and last elements of l are always 1 (since words are
# surrounded by syllable boundaries)

# fill dict with key-value pairs where key is an entry from entries with the
#  hyphens removed, value is a list representing syllable boundaries as
# described above
...
pre_sep_dict = {}  # map between words that have been hand-annotated and their
# annotations
...
vowel_seq_dict = {}  # map between sequences of three and for vowels and their
# syllabifications [modelled after Karlsson 1985: (2b), but using Karlsson 1982
# T-5/T-7 to deal with 'ieu', 'uoi']
VOWEL_SEQUENCES = [
    'ai-oi', 'ai-ui', 'au-oi', 'eu-oi', 'ie-oi', 'ie-ui', 'oi-oi', 'oi-ui',
    'uo-ui', 'yö-yi', 'a-ei', 'a-oi', 'e-ai', 'e-oi', 'e-äi', 'e-öi', 'i-ai',
    'i-au', 'i-oi', 'i-äi', 'i-öi', 'o-ai', 'u-ai', 'u-ei', 'u-oi', 'y-ei',
    'y-äi', 'ä-yi', 'ä-öi', 'ai-a', 'ai-e', 'ai-o', 'ai-u', 'au-a', 'au-e',
    'eu-a', 'ie-a', 'ie-o', 'ie-u', 'ie-y', 'i-o-a', 'i-o-e', 'i-ö-e', 'i-ö-ä',
    'iu-a', 'iu-e', 'iu-o', 'oi-a', 'oi-e', 'oi-o', 'oi-u', 'ou-e', 'ou-o',
    'u-e-a', 'ui-e', 'uo-a', 'uo-u', 'y-e-ä', 'yö-e', 'äi-e']
...
# applied Karlsson (3c); only checks for 'ien', since others are handled by
# vowel splitting rules
...
def apply_3c(word, boundary_list):
    ...
    if len(word) > seq_len:
        ...
        if word[-seq_len:] == sequence and word[-(seq_len+1)] != 't':
            ...
            boundary_list[-3] = 1  # last entry is for word-final syllable
            # boundary
...
# apply rule T-4 from Karlsson 1982 to two vowels, assuming the word is already
# syllabified
def apply_t4(word, boundary_list):

    # check for rule application at syllable boundary (including word end);
    # first possible boundary at index 3 (VVC-)
    for i in range(3, len(word)):

        if boundary_list[i] == 1:

            # if syllable ends in a T-4 diphthong followed by a consonant,
            # introduce split in former diphthong
...
def separate_vowels(vowels, boundary_list, start):
    ...
    if v_len == 2 and not is_inseparable_vowels(vowels):
        ...
        if vowels in vowel_seq_dict:
            ...
            # store information from vowel sequence dictionary; ignore first
            # entry, as the dictionary does not know if a syllable boundary
            # precedes the vowel sequence
            ...
            if boundary != -1:
                ...
                # if long vowel starts the sequence, separation should precede
                # the third vowel; otherwise it should procede the location of
                # the long vowel
                if boundary == 0:

                    boundary = 2
                    separate_vowels(
                        vowels[boundary:], boundary_list, start + boundary
            ...
            else:  # if no such sequence, simply separate all separable VV
                # sequences
                ...
                for i in range(len(vowels)-1):
                    ...
                    if not is_inseparable_vowels(vowels[i:i+2]):
                        ...
                        boundary_list[start + (i + 1)] = 1  # insert boundary
                        # before the second vowel


# return the syllabification of word, preserving capitalization; syllable
# boundaries are placed at the start and end of the word
def make_syllables(word):

    entry = word.lower()
    boundary_list = [1]

    # introduces annotations, but will still be syllabified so that only
    # partial annotations are required
    ...
    # syllable separator added to ensure that final vowel sequence is
    # syllabified
...
# return a string with the syllable boundaries represented in syllabified_word
# but the capitalization represented in original_word
...
def make_splits(word, boundary_list):  # TODO

    # stores the location of the start and end of the longest vowel sequence
    # encountered so far
    v_seq_start = 0
    v_seq_end = 0
    ...
    for i in range(len(word)):
        ...
        if is_vowel(word[i]):  # continuing or starting vowel sequence
            ...
            if v_seq_end - v_seq_start == 1:
                ...
                for onset_length in onset_lengths:
                    ...
                    # if encounter a good boundary, only insert separator if
                    # not already present; break regardless so that basic CV
                    # won't apply if appropriate cluster exists
                    if cluster_start >= 0 and \
                            consonantal_onset(word[cluster_start:i]):
        ...
        # vowel sequence interrupted; if there is a sequence to be split, deal
        # with it
        else:
            ...
            if v_seq_end - v_seq_start > 1:
                ...
                separate_vowels(
                    word[v_seq_start:v_seq_end],
                    boundary_list,
                    v_seq_start
                    )

            v_seq_start = v_seq_end = i+1  # vowel sequence (if any) starts
            # after current index
